...
app.post('/api/approvals/reject', isAdmin, async (req, res) => {
    ...
    try {
        ...
        logAction(username, 'UPDATE', type.toUpperCase(), id, `Rejected item with reason: ${reason}`);
...
app.post('/api/generate-2fa', (req, res) => {
    ...
    db.query('SELECT username, email FROM users WHERE id = ?', [userId], (err, results) => {
        ...
        const serviceName = "InventarioPro";
        const otpauth = authenticator.keyuri(user.email || user.username, serviceName, secret);

        db.query('UPDATE users SET twoFASecret = ? WHERE id = ?', [secret, userId], (updateErr) => {
            if (updateErr) return res.status(500).json({ message: "Failed to save 2FA secret" });
            res.json({ secret: secret, qrCodeUrl: otpauth });
        });
    });
});
    ...
    db.query('SELECT twoFASecret FROM users WHERE id = ?', [userId], (err, results) => {
        ...
        if (isValid) {
            db.query('UPDATE users SET is2FAEnabled = TRUE WHERE id = ?', [userId], (updateErr) => {
                if (updateErr) return res.status(500).json({ message: "Failed to enable 2FA" });
...
app.post('/api/disable-2fa', (req, res) => {
    const { userId } = req.body; // Assuming the user's own ID from a verified session
...
app.post('/api/disable-user-2fa', isAdmin, (req, res) => {
    const { userId } = req.body; // Admin provides the user ID to disable
    db.query('UPDATE users SET is2FAEnabled = FALSE, twoFASecret = NULL WHERE id = ?', [userId], (err) => {
        if (err) return res.status(500).json({ message: "Failed to disable 2FA for user" });
        res.status(200).json({ message: "2FA disabled successfully for user" });
    });
});
...
app.post('/api/settings', isAdmin, async (req, res) => {
    ...
    try {
        ...
        // Return the current state of lastAbsoluteUpdateTimestamp and hasInitialConsolidationRun
        const [rows] = await connection.query("SELECT config_key, config_value FROM app_config WHERE config_key IN ('lastAbsoluteUpdateTimestamp', 'hasInitialConsolidationRun')");
        const currentSettings = rows.reduce((acc, row) => {
            acc[row.config_key] = row.config_value;
            return acc;
        }, {});

        res.json({ success: true, message: 'Configurações salvas com sucesso.', ...currentSettings });
    } catch (err) {
        await connection.rollback();
...
app.get('/api/database/backup-status', isAdmin, (req, res) => {
    const backupFile = path.join(BACKUP_DIR, `backup-${DB_DATABASE}.sql.gz`);
...
app.post('/api/database/backup', isAdmin, (req, res) => {
    ...
    const backupFile = path.join(BACKUP_DIR, `backup-${DB_DATABASE}.sql.gz`);
    const command = `mysqldump --host=${DB_HOST} --user=${DB_USER} --password=${DB_PASSWORD} ${DB_DATABASE} | gzip > ${backupFile}`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error(`Backup failed: ${error.message}`);
            return res.status(500).json({ success: false, message: `Backup falhou: ${error.message}` });
        }
        if (stderr) {
            console.error(`Backup stderr: ${stderr}`);
            // Don't fail on stderr for mysqldump, as it often contains warnings.
        }
        logAction(username, 'UPDATE', 'DATABASE', null, 'Database backup created');
        res.json({ success: true, message: `Backup do banco de dados criado com sucesso em ${backupFile}.`, backupTimestamp: new Date().toISOString() });
    });
});
...
app.post('/api/database/restore', isAdmin, (req, res) => {
    ...
    const command = `gunzip < ${backupFile} | mysql --host=${DB_HOST} --user=${DB_USER} --password=${DB_PASSWORD} ${DB_DATABASE}`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error(`Restore failed: ${error.message}`);
            return res.status(500).json({ success: false, message: `Restauração falhou: ${error.message}` });
        }
        if (stderr) {
            console.warn(`Restore stderr: ${stderr}`);
        }
        logAction(username, 'UPDATE', 'DATABASE', null, 'Database restored from backup');
...
app.post('/api/database/clear', isAdmin, async (req, res) => {
    ...
    try {
        ...
        // List all tables except 'migrations'
        ...
        // Reset hasInitialConsolidationRun
        await connection.query(
            "INSERT INTO app_config (config_key, config_value) VALUES ('hasInitialConsolidationRun', 'false') ON DUPLICATE KEY UPDATE config_value = 'false';"
        );

        // Re-insert admin user
        const adminPassword = bcrypt.hashSync("marceloadmin", SALT_ROUNDS);
        await connection.query(
            `INSERT INTO users (username, realName, email, password, role) VALUES ('admin', 'Admin', 'admin@example.com', ?, 'Admin')
             ON DUPLICATE KEY UPDATE password = ?, realName = 'Admin', email = 'admin@example.com', role = 'Admin';`,
            [adminPassword, adminPassword]
        );

        await connection.commit();
        logAction(username, 'DELETE', 'DATABASE', null, 'Cleared all database tables and reset admin user');
        res.json({ success: true, message: "Banco de dados zerado com sucesso. O usuário 'admin' foi restaurado." });
